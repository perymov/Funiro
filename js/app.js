	...
	function addToCart(productButton, productId) {
		...
		if (!productButton.classList.contains('_hold')) {
			...
			const cart = document.querySelector('.cart-header__icon'),
				...
				productImage = product.querySelector('.item-product__image');

			const productImageFly = productImage.cloneNode(true);

			const productImageFlyWidth = productImage.offsetWidth,
				productImageFlyHeight = productImage.offsetHeight,
				productImageFlyTop = productImage.getBoundingClientRect().top,
				productImageFlyLeft = productImage.getBoundingClientRect().left;
			...
			const cartFlyLeft = cart.getBoundingClientRect().left,
				cartFlyTop = cart.getBoundingClientRect().top;
		...
		if (productAdd) {
			...
			if (!cartProduct) {
				const product = document.querySelector(`[data-pid="${productId}"]`),
					cartProductImage = product.querySelector('.item-product__image').innerHTML,
					cartProductTitle = product.querySelector('.item-product__title').innerHTML,
					cartProductContent = `
		...
		data.products.forEach(item => {
			...
			if (labels) {
				labels.forEach(labelitem => {
					productTemlateLabelsContent += `
					<div class="item-product__labels">
						<div class="item-product__label item-product__label_${labelitem.type}">${labelitem.value}</div>
					</div>
					`;
				});
			}
				...
				<article data-pid="${id}" class="products__item item-product">
					...
					<div class="item-product__body">
						...
						<div class="item-product__prices">
							...
							${priceOld ? `<div class="item-product__price item-product__price_old">${priceOld}</div>` : ''}
						</div>
						<div class="item-product__actions actions-product">
							<div class="actions-product__body">
								<button class="actions-product__button btn btn_white">Add to cart</button>
								<a href="${shareUrl}" class="actions-product__link _icon-share">Share</a>
								<button class="actions-product__link _icon-favorite">Like</button>
							</div>
						</div>
					</div>
				</article>
	...
	if (furniture && !isMobile.any()) {
		...
		function setMouseGalleryStyle() {
			...
			const furnitureDifferent = furnitureItemsWidth - furniture.offsetWidth,
				distX = Math.floor(coordXprocent - positionX);

			positionX = positionX + (distX * speed);
			let position = furnitureDifferent / 200 * positionX;

			furnitureItems.style.cssText = `
		...
		furniture.addEventListener('mousemove', function (e) {
			...
			//Вычисляем ноль когда курсор находится посередине
			const coordX = e.pageX - furnitureWidth / 2;

			//Вычисляем проценты когда курсор от середины доходит до левого или правого края на 100 процентов 
